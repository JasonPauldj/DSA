...
public class BST {
    ...
    BSTNode sortedArrayToBST(int[] arr, int start, int last) {
        ...
        BSTNode right = sortedArrayToBST(arr, mid + 1, last);

        BSTNode node = new BSTNode(arr[mid]);
        node.replaceLeft(left);
        node.replaceRight(right);

        return node;
    }

    BSTNode floor(int key, BSTNode root){
        if(root==null)
            return null;
        if(key < root.element())
        ...
        BSTNode node = floor(key, root.right());
        if(node!=null)
            return node;
        return root;
    }

    BSTNode ceiling(int key, BSTNode root){
        if(root==null)
            return null;
        if(key > root.element())
        ...
        BSTNode node = ceiling(key, root.left());
        if(node!=null)
            return node;
        return root;
    }
